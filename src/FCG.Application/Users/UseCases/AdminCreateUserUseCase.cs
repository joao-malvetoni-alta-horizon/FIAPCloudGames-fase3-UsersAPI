...
public class AdminCreateUserUseCase(
    ...
    IIntegrationEventPublisher eventPublisher) : IAdminCreateUserUseCase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var userRegisteredEvent = new UserRegisteredEvent(
            user.Id, user.Name.Value, user.Email.Address);
        await eventPublisher.PublishAsync(userRegisteredEvent, cancellationToken);

        return new AdminCreateUserResponse(user.Id, user.Name.Value, user.Email.Address, user.Role);
    }
}